...
import { validate, getNode, isStyleSheet, getDeepestNode } from "./util";
...
export function prevItemHTML(
	...
	if (
		offset > currentNode.open.start + currentNode.name.length + 1 &&
	...
	if (node && node.open) {
		...
		let selectionEnd =
			node.type === "comment"
				...
				: selectionStart.translate(0, node.name.length);
