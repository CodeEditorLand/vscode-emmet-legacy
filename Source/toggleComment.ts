	...
	if (node.type === "comment") {
		...
		rangesToUnComment.push(
			new vscode.Range(
				...
				document.positionAt(node.end - endCommentHTML.length),
			),
		);
...
function getRangesToUnCommentStylesheet(
	rootNode: Node,
	selectionStart: number,
	selectionEnd: number,
	document: vscode.TextDocument,
	selectionInsideComment: boolean,
): vscode.Range[] {
	...
	rootNode.comments.forEach((comment) => {
		...
		if (selectionInsideComment) {
			...
			foundComment =
				selectionStart <= comment.start && selectionEnd >= comment.end;
		}
