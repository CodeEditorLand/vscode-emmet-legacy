	...
	implements vscode.CompletionItemProvider
{
	public provideCompletionItems(
		...
		return Promise.resolve(
			new vscode.CompletionList(completionItems, true),
		);
	}
}
...
function getAbbreviationSuggestions(
	syntax: string,
	prefix: string,
	skipExactMatch: boolean,
) {
	if (
		!vscode.workspace.getConfiguration("emmet")["suggestAbbreviations"] ||
		!prefix ||
		isStyleSheet(syntax)
	) {
		return [];
